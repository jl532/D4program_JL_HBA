...
for eachFileName in singleArrayFileNames:
    ...
    if circlesRaw is None:
        print("no circles found at all: " + fileName)
        finalCircleOutput.append([[-99, -99, -99, -99, fileName]]) # added in extra list brackets because wanted to keep the list hierarchy consistent between no circles found and circles found
    else:
        circlesRaw = np.uint32(np.around(circlesRaw))
        numberOfCirclesFound = len(circlesRaw[0])
        print("Preliminary default search found " + str(numberOfCirclesFound) + " spots: "+ fileName)
        ...
        for eachCircle in sortedCircles:
            ...
            finalCircle = [eachCircle.tolist()[0] + indexOfMax[1] + 55] + [eachCircle.tolist()[1] + indexOfMax[0] + 55] + [eachCircle.tolist()[2]] + [round(avgIntensity,3)] + [fileName]
            finalCircleInfo.append(finalCircle)
        cv2.imwrite("algorithm_output+" + fileName, verImg)
        finalCircleOutput.append(finalCircleInfo)
        #print(finalCircleInfo)

outputCSVFileName = "whateverOutput.csv"
with open(outputCSVFileName, 'w', newline='') as outputCSV:
    csvWriter = csv.writer(outputCSV, quoting=csv.QUOTE_NONNUMERIC)
    for eachArray in finalCircleOutput:
        csvWriter.writerow([eachArray])
